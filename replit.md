...
- **100% open-source stack** (no proprietary services in the default path):
  - STT — `faster-whisper` (MIT)
  - LLM — Qwen2.5-7B-Instruct via Ollama (Apache 2.0)
  - TTS — `Indic-Parler-TTS` from AI4Bharat via the Hugging Face `parler-tts` library (Apache 2.0). Premium, prompt-driven, studio-grade Tamil/Indic voices.
  - Output is peak-normalized to ~ -3 dBFS for consistent premium loudness.
  - Coqui XTTS-v2 was removed in favour of Indic-Parler-TTS because XTTS-v2 ships under the Coqui Public Model License (non-commercial) — not a true OSS license. Voice cloning via reference WAV is therefore gone; voices are now created and modified via natural-language prompts only (which is what the user actually wanted anyway).
  - `engineFallbackChain` default is now `'local'` (no Sarvam/ElevenLabs in the default chain). Operators can opt into proprietary fallbacks by editing this setting.
- **Voice Lab (full ElevenLabs-style page at `/voice-lab`)**: dedicated route under sidebar group "Voice AI". Hero header, sticky "Try it" preview text bar, tabs (My Library / Voice Design), search + language/gender/tag filters, responsive grid of voice cards with gradient avatar (deterministic from voice id), inline ▶ preview with animated waveform, default-voice badge, ⋯ menu (Play / Edit / Copy ID / Delete), edit modal with tags/age/accent/use-case + tag preset chips. Voice Design tab generates 3 audio variants (Calm / Confident / Warm) from a base description prompt — operator A/B/Cs the previews, picks one, saves to library. Voice metadata: `tags[]`, `useCase`, `age`, `accent`, `favourite`, `updatedAt`. Inference endpoints: `GET /voices`, `POST /voices` (form fields incl. tags csv), `PATCH /voices/{id}` (rename/retune/retag), `DELETE /voices/{id}`, `POST /tts/preview`, `POST /tts/design` (returns 3 base64-WAV variants in JSON). Node mirrors at `/webhook/local-voices` (GET/POST/PATCH/DELETE/preview/design) with webhook-token auth on writes. Backwards-compat: legacy Voice Lab card under Settings remains for quick add. `voice_store.py` exposes `save_voice`, `update_voice`, `delete_voice`, `list_voices`, `_normalise()` helper, and `DESIGN_VARIANTS` constant.

- **Multi-Agent system (`/agents` page + `/api/agents` API)**: each agent is a self-contained voice persona with name, avatar (emoji), description, voice (linked to Voice Lab), language, greeting, system prompt, engine (local/sarvam/elevenlabs/kuralai), LLM model + temperature + max tokens, conversation mode (freeform/guided), tags, default flag. Agents stored in `data/agents.json` via `src/services/agentsService.js` (file-based, no migrations). Routes (`src/routes/agents.routes.js`): JWT-protected GET/POST/PATCH/DELETE + `GET /default`. Dashboard page features: hero, template strip (3 starter agents — Tamil Collections / Customer Support / Lead Qualifier) shown when library is small, search + engine filter, list of agent rows with avatar/voice/language/mode/model meta, ▶ preview greeting, Edit / Set Default / Delete actions, tabbed editor modal (Persona / Voice & Language / Brain / Tags & Defaults) with avatar emoji picker, voice picker grid with per-voice ▶, slider for temperature, conversation-mode dropdown. Agents wired into call flow via `src/services/localStream.js` — Session resolves agent by `meta.agentId` (or default), populates `meta.localTtsVoice / localVoiceDescription / localGreeting / localSystemPrompt / languageCode` if not already set, so per-call overrides win. Default seed agent "Samuthra" auto-created on first read. To run a call as a specific agent, set `callMeta.agentId` when initiating. Sidebar gains "Voice AI" group above "Manage" containing Agents + Voice Lab. New API client modules: `agentsApi`, `voicesApi` in `dashboard/src/api/client.js`.
