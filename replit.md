...
- **Voice Lab (ElevenLabs-grade UX, OSS internals)**: Settings UI card lists every voice with a `PARLER` badge, its underlying style description, ▶ preview, and delete. New voices are added via a single form (voice ID, display name, language, gender, style prompt). Inference server endpoints: `GET /voices`, `POST /voices` (multipart, `description` required), `DELETE /voices/<id>`, `POST /tts/preview` (≥22.05 kHz WAV). Node mirrors via `/webhook/local-voices` with multer + webhook-token auth on writes. Voice metadata lives in `inference-server/engines/voice_store.py` (extracted from the deleted XTTS module).
